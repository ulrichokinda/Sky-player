...
export const Player: React.FC<PlayerProps> = ({ 
  ...
  const groups = Array.from(new Set(channels.map(c => c.group || 'Général'))).sort();

  const filteredChannels = channels.filter(c => {
    const matchesSearch = c.name.toLowerCase().includes(searchQuery.toLowerCase()) || c.group?.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesGroup = selectedGroup === 'ALL' || (c.group || 'Général') === selectedGroup;
  ...
  useEffect(() => {
    ...
    if (isMixedContent) {
      console.log('Mixed Content detected: Routing through proxy', finalUrl);
    ...
    if (isM3U8) {
      if (Hls.isSupported()) {
          ...
          if (data.fatal) {
            switch(data.type) {
              case Hls.ErrorTypes.NETWORK_ERROR:
                throwError("Erreur réseau: Reconnexion en cours (Réseau faible)...");
                hls?.startLoad();
                break;
              ...
              default:
                hls?.destroy();
                throwError("Erreur fatale: Flux non supporté ou connexion instable.");
                break;
            }
          }
        });
        ...
        video.addEventListener('error', () => throwError("Erreur de connexion au serveur."));
      }
      ...
      try {
        mpegtsPlayer = mpegts.createPlayer({ 
          ...
          stashInitialSize: 1024, // 1MB for slower connections
          lazyLoad: false,
                ...
                <div className="flex gap-2 overflow-x-auto pb-2 custom-scrollbar">
                  <button
                    ...
                    className={`shrink-0 px-4 py-2 rounded-xl text-xs font-bold transition-all ${
                      selectedGroup === 'ALL' ? 'bg-primary text-black' : 'bg-white/5 text-zinc-400 hover:bg-white/10 hover:text-white'
                    }`}
                  >
                    Toutes les chaînes
                  </button>
                  {groups.map((g, idx) => (
                    <button
                      ...
                      className={`shrink-0 px-4 py-2 rounded-xl text-xs font-bold transition-all ${
                        selectedGroup === g ? 'bg-primary text-black' : 'bg-white/5 text-zinc-400 hover:bg-white/10 hover:text-white'
                      }`}
                    >
                      {g}
                    </button>
                ...
                {filteredChannels.length === 0 ? (
                  ...
                  return (
                    <button
                      key={index}
                      onClick={() => {
                        onChannelSelect?.(originalIndex >= 0 ? originalIndex : index);
