...
export const ResellerPanel: React.FC<ResellerPanelProps> = ({ activeTab, setActiveTab }) => {
  ...
  const handleActivate = async (e: React.FormEvent) => {
    ...
    if (!validateMacAddress(targetMac)) {
      notify("Adresse MAC invalide. Format: 00:11:22:33:44:55 ou format étendu", "error");
      return;
    }
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      ...
      const response = await api.initiateJOboostCheckout({
        userId,
      ...
      if (response.payment_url) {
        ...
        notify("Erreur lors de l'initialisation du paiement", "error");
      }
  ...
  const handleCheckMac = async () => {
    ...
    setLoading(true);
    try {
      // Call the API service to check the actual MAC status in Firestore
      const res = await api.checkMacStatus(checkMacInput);
      if (res.active && res.activation) {
        setCheckResult({
          mac: checkMacInput,
          status: 'Actif',
          last_server: res.activation.playlist_url ? 'M3U Link' : res.activation.xtream_host ? res.activation.xtream_host : 'Aucun',
          expiry: res.activation.createdAt ? new Date(res.activation.createdAt?.seconds * 1000).toLocaleDateString() : 'Actif à vie'
        });
      } else {
         setCheckResult({
          mac: checkMacInput,
    ...
    try {
      ...
      const response = await fetch('/api/ai-validation', {
        ...
        body: JSON.stringify({
          aiImage,
          promptText: prompt,
          expectedPrice: selectedPack.price,
          currency,
          selectedPack
        })
      });
      ...
      if (!response.ok) {
        throw new Error(jsonResponse.error || "Erreur lors de la validation IA.");
      }
        ...
        {activeTab === 'credits' && (
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                { qty: 10, basePrice: 12750, popular: false, desc: '1 crédit = 1 an de validité' },
                { qty: 20, basePrice: 23750, popular: false, desc: '1 crédit = 1 an de validité' },
                { qty: 50, basePrice: 45000, popular: true, desc: '1 crédit = 1 an de validité' },
              ].map((pack) => {
                const convertedPrice = getConvertedPrice(pack.basePrice, userCountry);
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 gap-3">
                          ...
                          >
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest">JOboost-Cash</span>
                            <span className="text-[7px] font-black uppercase tracking-[0.2em] opacity-40">Mobile Money & Cartes</span>
                          </button>
                        </div>
                      </div>
