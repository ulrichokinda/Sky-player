...
import { Tv, Copy, ArrowRight, ShieldAlert, Gift, Wifi, WifiOff, RotateCcw, Loader2, Star, Activity, Check, X, Edit2, List } from 'lucide-react';
...
export const SimpleUserView: React.FC<SimpleUserViewProps> = ({ onNotify }) => {
  ...
  // New state to manage the active IPTV view
  const [activeView, setActiveView] = useState<'dashboard' | 'player'>('dashboard');
  ...
  if (activeView === 'player' && channels.length > 0) {
            ...
            <div className="space-y-6 w-full pb-8">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <button 
                  ...
                  className="bg-primary/10 border border-primary/30 p-6 rounded-3xl flex flex-col items-center gap-4 hover:bg-primary hover:text-black transition-all group"
                >
                  <div className="p-4 bg-primary text-black rounded-2xl group-hover:scale-110 transition-transform">
                    <Tv size={32} />
                  </div>
                  <div className="flex flex-col items-center gap-1">
                    <span className="text-sm font-black uppercase tracking-widest text-primary group-hover:text-black">Chaînes en Direct</span>
                    <span className="text-[10px] font-bold text-zinc-500 group-hover:text-black/60">{channels.filter(c => !c.url.includes('/movie/') && !c.url.includes('/series/')).length} chaînes</span>
                  </div>
                </button>

                <button 
                  onClick={() => {
                    const firstMovie = channels.findIndex(c => c.url.includes('/movie/'));
                    if (firstMovie > -1) {
                      setCurrentChannelIndex(firstMovie);
                      setActiveView('player');
                    } else {
                       onNotify("Aucun film dans cette playlist.", 'info');
                    }
                  }}
                  className="bg-zinc-900/50 border border-zinc-800 p-6 rounded-3xl flex flex-col items-center gap-4 hover:bg-zinc-800 transition-all group"
                >
                  <div className="p-4 bg-zinc-800 text-zinc-400 group-hover:text-white rounded-2xl group-hover:scale-110 transition-transform">
                    <Gift size={32} />
                  </div>
                  <div className="flex flex-col items-center gap-1">
                    <span className="text-sm font-black uppercase tracking-widest">Films (VOD)</span>
                    <span className="text-[10px] font-bold text-zinc-500">{channels.filter(c => c.url.includes('/movie/')).length} films</span>
                  </div>
                </button>

                <button 
                  onClick={() => {
                    const firstSeries = channels.findIndex(c => c.url.includes('/series/'));
                    if (firstSeries > -1) {
                      setCurrentChannelIndex(firstSeries);
                      setActiveView('player');
                    } else {
                       onNotify("Aucune série dans cette playlist.", 'info');
                    }
                  }}
                  className="bg-zinc-900/50 border border-zinc-800 p-6 rounded-3xl flex flex-col items-center gap-4 hover:bg-zinc-800 transition-all group"
                >
                  ...
                  <div className="flex flex-col items-center gap-1">
                    ...
                    <span className="text-[10px] font-bold text-zinc-500">{channels.filter(c => c.url.includes('/series/')).length} épisodes</span>
                  </div>
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <button 
                  onClick={() => {
                    ...
                    const firstFav = channels.findIndex(c => favs.includes(c.url));
                    if (firstFav > -1) {
                      setCurrentChannelIndex(firstFav);
                      setActiveView('player');
                    } else {
                      onNotify(`Vous avez ${favs.length} favoris, mais aucun dans la playlist courante.`, 'info');
                    }
                  }}
