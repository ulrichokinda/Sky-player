...
export const api = {
  ...
  async initiatePayment(data: { userId: string; amount: number; phoneNumber: string; credits_purchased: number; provider: string; methodId: string; target_mac?: string; plan_id?: string }) {
  ...
  async initiateJOboostCheckout(data: {
    userId?: string;
    amount: number;
    phoneNumber?: string;
    credits_purchased?: number;
    ...
    const payload = {
      mac: data.target_mac || '',
      amount: data.amount,
      packageId: data.plan_id || (data.credits_purchased ? `${data.credits_purchased}cr` : 'Premium'),
      phone: data.phoneNumber || '',
      userId: data.userId || '',
      credits_purchased: data.credits_purchased || 0
    };
    
    const response = await nativeFetch('/api/v1/payments/checkout', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async checkMacStatus(mac: string): Promise<{ active: boolean; activation?: any; error?: string }> {
    try {
      if (!mac) return { active: false, error: "Adresse MAC requise." };
      
      // Normaliser la MAC comme sur le serveur (minuscule, sans espaces/delégateurs)
      const cleanLowerMac = mac.toLowerCase().replace(/[^a-f0-9]/g, '');
      const cleanUpperMac = mac.toUpperCase().replace(/[^a-f0-9]/g, '');
      const origMac = mac.trim();
      
      const activationsRef = collection(db, 'activations');
      let snapshot;
      
      // Essai 1 : Format standard (minuscule nettoyée)
      try {
        const q = query(activationsRef, where('target_mac', '==', cleanLowerMac));
        snapshot = await getDocs(q);
      } catch (e) {
        console.warn("getDocs standard failed");
      }
      
      // Essai 2 : Format Majuscule nettoyée (si legacy)
      if (!snapshot || snapshot.empty) {
        try {
          const q = query(activationsRef, where('target_mac', '==', cleanUpperMac));
          snapshot = await getDocs(q);
        } catch (e) {}
      }
      
      // Essai 3 : Format original brut (si saisi manuellement avec colons ou espaces dans la base de données)
      if (!snapshot || snapshot.empty) {
        try {
          const q = query(activationsRef, where('target_mac', '==', origMac));
          snapshot = await getDocs(q);
        } catch (e) {}
      }
      ...
      if (snapshot && !snapshot.empty) {
        ...
        // REST Fallback for extreme cases (using cleanLowerMac as primary check)
        try {
            ...
            structuredQuery: {
              ...
              where: {
                fieldFilter: {
                  ...
                  op: 'EQUAL',
                  value: { stringValue: cleanLowerMac }
                }
              }
            }
          };
                ...
                structuredQuery: {
                  ...
                  where: {
                    fieldFilter: {
                      field: { fieldPath: 'target_mac' },
                      op: 'EQUAL',
                      value: { stringValue: cleanUpperMac }
                    }
                  }
                }
              };
              const restResponseUpper = await nativeFetch(restUrl, {
                method: 'POST',
                body: JSON.stringify(restBodyUpper)
              });
              if (restResponseUpper.ok) {
                const restDataUpper: any = await restResponseUpper.json();
                if (restDataUpper && restDataUpper.length > 0 && restDataUpper[0].document) {
                  const doc = restDataUpper[0].document;
                  foundId = doc.name.split('/').pop();
                  const mapFields = (fields: any) => {
                    const res: any = {};
                    for (const key in fields) {
  ...
  async updateCurrentChannel(mac: string, channelName: string) {
    return this.sendHeartbeat({ mac, channel: channelName });
  },
