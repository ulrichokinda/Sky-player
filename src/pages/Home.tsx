...
import { ResellerPricingModal } from '../components/ResellerPricingModal';
...
export const Home = () => {
  ...
  const [isResellerPricingOpen, setIsResellerPricingOpen] = useState(false);
  ...
  const handleDownloadAPK = async () => {
    try {
      const { storage, ref, getDownloadURL } = await import('../firebase');
      
      const fileNames = ['SkyPlayer.apk', 'skyplayer.apk', 'app-release.apk', 'app.apk'];
      let downloadUrl = '';
      ...
      if (downloadUrl) {
        ...
        alert("L'APK n'est pas encore disponible ou n'a pas été téléchargé sur le serveur (Firebase Storage). Veuillez contacter l'administrateur.");
      }
    } catch (error) {
      alert("Erreur lors du téléchargement de l'APK. Assurez-vous que Storage est correctement configuré.");
    }
  };
  ...
  const handlePlanClick = (e: React.MouseEvent, plan: string) => {
    e.preventDefault();
    if (!user) {
      // Show direct checkout instead of redirecting to register
      setCheckoutPlanId(plan);
      setIsCheckoutOpen(true);
    } else {
      // Reseller user logic: go to standard payment page
  ...
  return (
    <div className="min-h-screen bg-black text-white overflow-x-hidden">
      ...
      />
      <ResellerPricingModal isOpen={isResellerPricingOpen} onClose={() => setIsResellerPricingOpen(false)} />
      ...
      <nav className="flex items-center justify-between py-6 px-4 md:px-8 sticky top-0 bg-black/80 backdrop-blur-md z-50 border-b border-white/5">
        ...
        <div className="hidden md:flex items-center gap-4">
          <Button variant="ghost" size="sm" onClick={() => setIsResellerPricingOpen(true)} className="text-zinc-400 font-bold border border-transparent hover:border-zinc-800">Tarifs Revendeur</Button>
          <Link to="/about">
          ...
          >
            <div className="flex flex-col gap-6">
              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => {setIsMenuOpen(false); setIsResellerPricingOpen(true);}} className="p-4 bg-primary/10 rounded-2xl border border-primary/20 text-center col-span-2">
                  <p className="text-primary font-bold text-sm">Tarifs Revendeur</p>
                </button>
              ...
              {user ? (
                <div className="pt-4">
                  <Link to="/dashboard" onClick={() => setIsMenuOpen(false)} className="block w-full">
                    <div className="flex items-center gap-4 p-5 bg-primary/10 rounded-2xl border border-primary/20 hover:bg-primary/20 transition-all">
                      <LayoutDashboard className="text-primary shrink-0" />
                      <div className="text-left">
                ...
                <div className="flex flex-col gap-6 pt-4 pb-8 w-full mt-2">
                  <Link to="/login" onClick={() => setIsMenuOpen(false)} className="block w-full relative z-10">
                    <div className="flex items-center gap-4 p-5 bg-zinc-900 rounded-2xl border border-zinc-800 shadow-xl hover:bg-zinc-800 transition-all">
                      <LogIn className="text-zinc-400 shrink-0" />
                      <div className="text-left">
                        <p className="font-bold text-white leading-tight">Connexion</p>
                        <p className="text-xs text-zinc-500 mt-1">Accédez à votre compte</p>
                      </div>
                    </div>
                  </Link>
                  <Link to="/register" onClick={() => setIsMenuOpen(false)} className="block w-full relative z-10">
                    <div className="flex items-center gap-4 p-5 bg-primary rounded-2xl border border-primary/30 shadow-xl shadow-primary/10 hover:bg-primary/90 transition-all">
                      <UserPlus className="text-black shrink-0" />
                      <div className="text-left">
                        <p className="font-bold text-black leading-tight">Devenir Revendeur</p>
                        <p className="text-xs text-black/60 mt-1">Lancez votre business</p>
                      </div>
                    </div>
                  </Link>
                </div>
              ...
              <div className="pt-6 border-t border-zinc-800">
                <Button fullWidth size="lg" icon={Download} onClick={handleDownloadAPK}>Télécharger APK</Button>
              </div>
            </div>
          </motion.div>
        )}
      ...
      <main className="relative z-10 max-w-6xl mx-auto mt-12 md:mt-24 text-center space-y-16 px-4 md:px-6">
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4 pt-8">
            <Button size="lg" className="bg-primary hover:bg-primary-dark w-full sm:w-auto px-10 text-lg font-black h-16" onClick={handleDownloadAPK} icon={Download}>Télécharger APK</Button>
        ...
        <div className="py-12 md:py-16 space-y-10">
          <div className="space-y-4 text-center">
            ...
            <div className="pt-4">
               <Button variant="outline" className="border-primary/50 text-primary hover:bg-primary hover:text-black font-bold" onClick={() => setIsResellerPricingOpen(true)}>
                 Découvrir les offres revendeurs
               </Button>
            </div>
          </div>
