  ...
  const handleDownloadAPK = async () => {
    try {
      ...
      if (downloadUrl) {
        ...
        alert("L'APK n'est pas encore disponible sur Firebase Storage.");
      }
    } catch (error) {
      alert("Erreur lors du téléchargement. Firebase Storage n'est pas configuré.");
    }
  };
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'Acheter des crédits':
        return <CreditShop user={user} onPaymentSuccess={() => {
        ...
        return (
          <div className="grid md:grid-cols-2 gap-6">
            <Card className="space-y-4">
              ...
              <Button variant="outline" fullWidth icon={ExternalLink} onClick={handleDownloadAPK}>Télécharger APK</Button>
            </Card>
            <Card className="space-y-4">
              <div className="flex items-center gap-3 text-primary">
                <Download size={24} />
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col md:flex-row overflow-x-hidden">
      ...
      <aside className={cn(
        ...
        <nav className="flex-1 flex flex-col gap-4 overflow-y-auto custom-scrollbar pt-6 pb-12 w-full px-2">
          {menuItems.map(item => (
            <button 
              key={item.name} 
              onClick={() => { setActiveTab(item.name); setIsSidebarOpen(false); }}
              className={cn(
                "flex items-center gap-4 w-full p-4 rounded-xl transition-all text-sm font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary mb-2",
                activeTab === item.name 
                  ? "bg-primary text-black shadow-lg shadow-primary/20 scale-105" 
                  : "text-zinc-400 hover:bg-zinc-800 hover:text-white",
                isSidebarCollapsed && "justify-center px-0 scale-100"
              )}
              title={isSidebarCollapsed ? item.name : undefined}
            >
              <item.icon size={22} className={cn("shrink-0", activeTab === item.name ? "text-black" : "text-zinc-500")} />
              {!isSidebarCollapsed && <span className="uppercase tracking-wider">{item.name}</span>}
              {!isSidebarCollapsed && activeTab === item.name && (
...
const CreditShop = ({ user, onPaymentSuccess }: { user: any; onPaymentSuccess: () => void }) => {
  ...
  const handleInitPayment = async () => {
    ...
    try {
      ...
      if (result && result.payment_url) {
        ...
        setStatusMessage("Fenêtre de paiement ouverte. Finalisez la transaction sur JOboost-Cash.");
        setStep('waiting');
      } else {
  ...
  return (
    <Card className="max-w-md mx-auto text-center space-y-8 p-12 animate-in zoom-in-95 duration-500">
      ...
      <div className="flex flex-col gap-4">
        <Button variant="primary" onClick={() => onPaymentSuccess()}>
