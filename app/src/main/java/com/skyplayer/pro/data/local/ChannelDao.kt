...
interface ChannelDao {
    ...
    @Query("SELECT * FROM channels WHERE type IN (:types) ORDER BY name ASC")
    fun getChannelsByTypes(types: List<ContentType>): Flow<List<Channel>>
    ...
    @Query("SELECT DISTINCT category FROM channels WHERE type IN (:types)")
    fun getCategoriesByTypes(types: List<ContentType>): Flow<List<String>>
    ...
    @Query("SELECT * FROM channels WHERE lastWatched IS NOT NULL ORDER BY lastWatched DESC LIMIT :limit")
